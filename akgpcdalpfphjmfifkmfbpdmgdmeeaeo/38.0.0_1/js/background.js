function isProduction() {
  return !chrome.extension.getURL("/").match(/iljllnndhakkoejiikidpbnlgloodoif/)
}
if (!isProduction()) {
  localStorage.server = "http://localhost:945"
  localStorage.uploadURL = "/upload3.asp"
} else {
  localStorage.server = "http://www.openscreenshot.com"
}
localStorage.uploadURL = localStorage.server + "/upload3.asp"
...
function toChecks() {
  if (!localStorage.created) {
    ...
    localStorage.skip37update = true
  }
  if (!localStorage.skip37update) {
    function a() {
      ...
      switch (data.data) {
        ...
        case "captureVisible":
          screenshot.captureVisible(
            $.extend({}, data, {
              callback: data.fast && oneClickShare,
              runCallback: data.fast,
            }),
          )
          break
        case "captureAll":
