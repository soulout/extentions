var screenshot = {
  ...
  captureRegion: function() {
    ...
    screenshot.tryGetUrl(function() {
      ...
      codeinjector.executeOnTab(
        ...
        true,
        function() {
          chrome.tabs.executeScript(screenshot.thisTabId, {
  ...
  editContent: function() {
    screenshot.tryGetUrl(function() {
      chrome.tabs.executeScript(
        screenshot.thisTabId,
        { allFrames: true, code: 'document.designMode="on"' },
        function() {
  ...
  load: function(callback) {
    screenshot.tryGetUrl(function() {
      ...
      callback = function() {
        window.setTimeout(
          realCallback,
          (parseInt(localStorage["delay"], 10) || 0) * 1000,
        )
      }
  ...
  ans: function(mess) {
    ...
    if (!mess && chrome.runtime.lastError) {
      if (screenshot.retries > 1 && screenshot.scroll) {
        api.callPopup({
          type: "message",
          message:
            "Sorry, we can not take a full screenshot of this webpage. This might be because it is not fully loaded. Please report this issue.",
        })
        return
    ...
    var cb = function(data) {
      ...
      if (mess.top || parseInt(mess.top) == 0) {
        screenshot.screens.push({
          left: parseInt(mess.left),
          top: parseInt(mess.top),
          data: data,
        })
      }
      if (mess.finish) {
    ...
    setTimeout(function() {
      ...
      capture()
      return
      // set active window to take screenshotas
      chrome.windows.update(
        screenshot.thisWindowId,
        { focused: true },
        function() {
          capture()
          // set active tab to take screenshots
          chrome.tabs.update(screenshot.thisTabId, { active: true }, capture)
        },
      )
    }, timeoutInterval)
  },
